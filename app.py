...
class MenuScene(Scene):
    ...
    def __init__(self):
        ...
        self.instruction_text = "Меню"

        self.btn_start = pg.Button(
            340,
            480,
            120,
            35,
    ...
    def draw(self, screen):
        ...
        pg.Text(
            275, HEIGHT / 2 - HEIGHT / 4, self.title_text, size=96, color="#7ab71e"
        ).draw(screen)

        # Отображение инструкции (центрировано)
        pg.Text(350, HEIGHT / 2 + 100, self.instruction_text, size=32, color="#c0e8ec").draw(
            screen
        )
...
class GameScene(Scene):
    ...
    def __init__(self):
        super().__init__("game")
        self.instruction_text = "A/D - влево/вправо, S - присесть, Q/E - телепортация вверх/вниз, Shift + A/D - бег, Space - прыжок"
    ...
    def update(self, dt):
        ...
        if keys[pygame.K_s] and on_ground:
            ...
            # Ходим или бегаем
                ...
                self.player.x -= (
                    self.run_speed if keys[pygame.K_LSHIFT] else self.speed
                ) * dt
                if on_ground:
                    self.player.play_animation(
                        "run" if keys[pygame.K_LSHIFT] else "walk"
                    )
        ...
        if not moving and not crouching and not jumping:
            self.idle_timer += dt
            if (
                self.idle_timer < IDLE_TO_STAND
                and self.player.get_current_animation() != "stance"
            ):
                self.player.play_animation("stance")
            elif (
                self.idle_timer >= IDLE_TO_STAND
                and self.player.get_current_animation() != "stand"
            ):
...
class FirstScene(GameScene):
    def __init__(self):
        ...
        self.initial_ground_y = (
            HEIGHT / 2 - (self.door.frame_size[1] * self.door.scale) / 2
        )
        self.door.set_position(self.initial_ground_x, self.initial_ground_y)
        self.close_key.set_position(
            self.initial_ground_x,
            self.initial_ground_y - (self.door.frame_size[1] * self.door.scale) / 2,
        )

        self.initial_ground_y = (
            HEIGHT / 2 - (self.sign.frame_size[1] * self.sign.scale) / 2
        )
    ...
    def draw(self, screen):
        ...
        pg.Text(20, 20, self.instruction_text, size=18, color=BLACK).draw(screen)
